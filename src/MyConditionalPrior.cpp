...
void MyConditionalPrior::from_prior(DNest4::RNG& rng)
{
    ...
    mu_fwidth = exp(log(1E-3)*(Data::f_max - Data::f_min) + log(1E3)*rng.rand());
    sig_fwidth = 2.0*rng.rand();
}
...
double MyConditionalPrior::perturb_hyperparameters(DNest4::RNG& rng)
{
    ...
    {
        mu_fwidth = log(mu_fwidth);
        mu_fwidth += log(1E3)*rng.randh();
        DNest4::wrap(mu_fwidth,
                        log(1E-3*(Data::f_max - Data::f_min)),
                        log(Data::f_max - Data::f_min));
        mu_fwidth = exp(mu_fwidth);
    }
    else
    {
        sig_fwidth += 2.0*rng.randh();
        DNest4::wrap(sig_fwidth, 0.0, 2.0);
    }
...
double MyConditionalPrior::log_pdf(const std::vector<double>& vec) const
{
    ...
    g = DNest4::Gaussian(log(mu_fwidth), sig_fwidth);
    if(vec[7] < 0.0)
        return -1E300;
    logp += -log(vec[7]) + g.log_pdf(log(vec[7]));
...
void MyConditionalPrior::to_uniform(std::vector<double>& vec) const
{
    ...
    g = DNest4::Gaussian(log(mu_fwidth), sig_fwidth);
    vec[7] = g.cdf(log(vec[7]));
}

void MyConditionalPrior::print(std::ostream& out) const
{
    out << mass_scale << ' ' << mu_width << ' ' << sig_width << ' ';
    out << mu_fwidth << ' ' << sig_fwidth << ' ';
}
